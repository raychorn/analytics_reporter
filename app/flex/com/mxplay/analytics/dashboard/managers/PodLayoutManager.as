...
package com.mxplay.analytics.dashboard.managers
{	
import com.mxplay.analytics.dashboard.events.LayoutChangeEvent;
import com.mxplay.analytics.dashboard.events.PodStateChangeEvent;
import com.mxplay.analytics.dashboard.view.DragHighlight;
...
[Event(name="update", type="com.mxplay.analytics.dashboard.events.LayoutChangeEvent")]

public class PodLayoutManager extends EventDispatcher
{
	public var id:String;
	public var selectedLayout:String;						// Stores the layout format.
	...
	public function updateLayout(tween:Boolean=true):void
	{
		...
		{
			//Vertical Layout
			//Remove first Pod from Items into sidePod
			//if (sidePod == null) {
			//	sidePod = items[len-1];
			//	sidePod.index = len-1;
			//	items.splice(len-1, 1);
			//}
			//Get Lenght of New Items
			len = items.length;
			var sqrt:Number = Math.floor(Math.sqrt(len));
			var numRows:Number = Math.ceil(len / sqrt);
			var numCols:Number = Math.ceil(len / numRows);
		}
		else
		{
			//Horizontal Layout
		...
		{			
			...
			//Alert.show("Index=" + i + "; targetX=" + targetX + "; targetY=" + targetY,"targets");
		...
		// Layout Side Items depending on LayoutStyle
		if (sidePod != null)
		{
			sidePod.height = availablePodHeight;
			targetX = Math.round((col+1) * itemWidth + POD_GAP);
			if (tween)
			{
				addResizeAndMoveToParallel(sidePod, parallel, availablePodWidth-itemWidth, availablePodHeight, targetX, 0);
			}
			else
			{
				sidePod.width = availablePodWidth-itemWidth;
				sidePod.x = targetX;
				sidePod.y = 0;
			}
			gridPoints[sidePod.index] = new Point(targetX, 0);
		}
