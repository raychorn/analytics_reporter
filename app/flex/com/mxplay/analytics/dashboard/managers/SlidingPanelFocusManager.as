 ...
 */
public class SlidingPanelFocusManager implements IFocusManager
{
     ...
     */
    public function SlidingPanelFocusManager(container:IFocusManagerContainer, popup:Boolean = false)
    {
        super();
    ...
    private function addFocusables(o:DisplayObject, skipTopLevel:Boolean = false):void
    {
        ...
        if (o is IFocusManagerComponent && !skipTopLevel)
        {
            ...
            {
            	// ERS
            	// When checking whether or not we want to add this component,
            	// don't call isTabVisible as this will trace up the parent containers
            	// and find the SlidingPanel container which has tabEnabled set to false
            	// to disable the parent object from tabbing around the children of the 
            	// sliding panel.
        ...
        if (o is DisplayObjectContainer)
        {
			...
			// ERS
			// Ignore the tabChildren setting of the container because we 
			// explicitly set that to false to prevent it's parent FocusManager
			// from adding components within the sliding panel.
            if (doc.tabChildren || o == _form)
            {
                var i:int;
